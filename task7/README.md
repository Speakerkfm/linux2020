...
/root
```
Добавим путь к openssl при билде nginx
```
...
```
Создадим каталог repo в директории nginx
```
...
Pool finished
```
Добавим `autoindex on;` в конфиг фаил nginx
...
```
Проверим, что репозиторий создался с помощью curl
```
...
mai                                mai-linux
```
Переустановим nginx из локального репозитория
```
...
Complete!
```
Установим percona-release из нашего репозитория и проверим, что все ок
```
